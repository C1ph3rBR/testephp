...
if (!empty($_POST)) {
    ...
    if ($validacao) {
        ...
        $sql = "UPDATE consuta  set nome = ?, endereco = ?, telefone = ?, email = ?, sexo = ? WHERE id = ?";
        $q = $pdo->prepare($sql);
    ...
    $pdo = Banco::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM consuta where id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($id));
    $data = $q->fetch(PDO::FETCH_ASSOC);
    $nome = $data['nome'];
    $doutor = $data['doutor'];
    $hora = date('d/m/Y H:i:s', strtotime($data['data_hora']));
...
<head>
    ...
    <title>Atualizar  do Paciente</title>
</head>
    ...
    <div class="span10 offset1">
        <div class="card">
            <div class="card-header">
                <h3 class="well"> Atualizar Contato do Paciente</h3>
            </div>
            <div class="card-body">
                <form class="form-horizontal" action="update.php?id=<?php echo $id ?>" method="post">


                   <div> Consuta com <?php echo $doutor; ?> dia <?php echo $hora; ?> </div>
